...
-   **Database Migration (In Progress):** Drizzle ORM with @neondatabase/serverless configured for Replit PostgreSQL. Schema defined in shared/schema.ts, connection tested via /api/db-test endpoint. Migration plan: Storage → Database → Auth with feature flags for dual-running and safe rollback. Currently Supabase still active for main data; Drizzle layer ready for incremental route migration.
