...
export interface Memory {
  ...
  content: string;
  embedding?: number[];
  memory_type: 'conversation' | 'insight' | 'preference' | 'presentation_topic' | 'design_preference' | 'goal' | 'milestone';
...
export class VectorMemoryService {
  ...
  async searchMemories(
    ...
    query: string,
    limit: number = 10,
    memoryTypes?: Memory['memory_type'][]
  ): Promise<Memory[]> {
    try {
      ...
      if (data && data.length > 0) {
        await this.updateAccessTracking(data.map((m: Memory) => m.id));
      }
  ...
  private combineResults(
    semanticResults: Memory[],
    recentResults: Memory[],
