-- ============================================================================
-- SLIDECOFFEE: Apply RLS Policies ONLY (No table creation)
-- Safe to run on existing schema - only adds missing policies
-- ============================================================================

-- ============================================================================
-- STEP 1: Enable RLS on all v2_ tables (safe if already enabled)
-- ============================================================================
...
-- ============================================================================
-- STEP 2: Drop existing policies (to avoid conflicts) and recreate
-- ============================================================================
...
-- v2_chat_messages policies
DROP POLICY IF EXISTS chat_messages_read_workspace ON v2_chat_messages;
DROP POLICY IF EXISTS chat_messages_insert_workspace ON v2_chat_messages;

CREATE POLICY chat_messages_read_workspace ON v2_chat_messages 
FOR SELECT USING (EXISTS (
  SELECT 1 FROM v2_presentations p
  JOIN v2_workspace_members wm ON p.workspace_id = wm.workspace_id
  WHERE p.id = v2_chat_messages.presentation_id
  AND wm.user_id = auth.uid()
));

CREATE POLICY chat_messages_insert_workspace ON v2_chat_messages 
FOR INSERT WITH CHECK (EXISTS (
  SELECT 1 FROM v2_presentations p
  JOIN v2_workspace_members wm ON p.workspace_id = wm.workspace_id
  WHERE p.id = v2_chat_messages.presentation_id
...
-- ============================================================================
-- DEPLOYMENT COMPLETE!
-- This script enabled RLS and applied 40+ policies across all tables
-- ============================================================================
