-- ============================================================================
-- SLIDECOFFEE: Apply RLS Policies ONLY (Type-Safe Version)
-- Handles text vs UUID type mismatches with explicit casts
-- ============================================================================
...
-- v2_support_tickets policies (TEXT user_id - requires casting)
...
-- v2_support_ticket_replies policies (TEXT user_id - requires casting)
...
FOR SELECT USING (EXISTS (
  ...
  AND (st.user_id = auth.uid()::text OR EXISTS (
    SELECT 1 FROM v2_users WHERE id = auth.uid() AND role = 'admin'
  ))
));
...
-- ============================================================================
-- DEPLOYMENT COMPLETE!
-- Applied 36 RLS policies with proper type casting
-- ============================================================================
