...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {    // Try Supabase authentication
    ...
    try {
      ...
      let user = await db.getUserByEmail(supabaseUser.email || '');
      
      // If user not in DB, create them
      if (!user) {
        console.log('[Auth] Creating new user:', supabaseUser.email);
        await db.upsertUser({
          openId: supabaseUser.id, // Use Supabase UUID as openId
        ...
        // Ensure default workspace is created for new user
        if (user) {
          console.log('[Auth] Creating default workspace for user:', user.id);
          await db.ensureDefaultWorkspace(user.id);
        }
      }
